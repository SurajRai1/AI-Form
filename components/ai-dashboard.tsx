...
import { Send, Plus, BarChart3, Settings as SettingsIcon, FileText, Eye, Download, Trash2, Edit, Users, TrendingUp, Clock, Star } from 'lucide-react';
...
const AIDashboard: React.FC = () => {
	const [activeTab, setActiveTab] = useState<'chat' | 'forms' | 'analytics' | 'settings'>('chat');
	...
	const handleSendMessage = async () => {
		...
		setTimeout(() => {
			...
			if (userMessage.content.toLowerCase().includes('create') || userMessage.content.toLowerCase().includes('build')) {
				setTimeout(() => {
		...
		<div className="flex flex-col h-full">
			...
			<div className="p-6 border-t bg-white/80 dark:bg-slate-900/80 backdrop-blur">
				...
				<div className="mt-3 text-xs text-gray-500 dark:text-gray-400 text-center">
					Try: "Create a registration form" or "Build a customer survey with rating scale"
				</div>
			</div>
		</div>
	);
	...
	const FormsLibrary = () => (
		<div className="p-6">
			<div className="flex justify-between items-center mb-6">
				<h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">My Forms</h2>
				<button
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
				{forms.map((form) => (
					<div key={form.id} className="bg-white/80 dark:bg-slate-900/80 border border-gray-200 dark:border-gray-700 rounded-xl p-6 hover:shadow-lg transition-shadow backdrop-blur">
						<div className="flex justify-between items-start mb-4">
							<div className="flex-1">
								<h3 className="font-semibold text-lg text-gray-900 dark:text-gray-100 mb-2">{form.title}</h3>
						...
						<div className="grid grid-cols-2 gap-4 mb-4">
							<div className="text-center">
								<div className="flex items-center justify-center space-x-1">
									<Users size={14} />
									<span className="text-sm font-medium text-gray-900 dark:text-gray-100">{form.responses}</span>
								</div>
							...
							<div className="text-center">
								<div className="flex items-center justify-center space-x-1">
									<Eye size={14} />
									<span className="text-sm font-medium text-gray-900 dark:text-gray-100">{form.views}</span>
								</div>
		...
		return (
			<div className="p-6">
					...
					<div className="bg-white/80 dark:bg-slate-900/80 p-6 border border-gray-200 dark:border-gray-700 rounded-xl backdrop-blur">
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-3xl font-bold text-gray-900 dark:text-gray-100">{forms.length}</p>
							</div>
					...
					<div className="bg-white/80 dark:bg-slate-900/80 p-6 border border-gray-200 dark:border-gray-700 rounded-xl backdrop-blur">
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-3xl font-bold text-gray-900 dark:text-gray-100">{totalResponses}</p>
							</div>
					...
					<div className="bg-white/80 dark:bg-slate-900/80 p-6 border border-gray-200 dark:border-gray-700 rounded-xl backdrop-blur">
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-3xl font-bold text-gray-900 dark:text-gray-100">{totalViews}</p>
							</div>
					...
					<div className="bg-white/80 dark:bg-slate-900/80 p-6 border border-gray-200 dark:border-gray-700 rounded-xl backdrop-blur">
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-3xl font-bold text-gray-900 dark:text-gray-100">{conversionRate}%</p>
							</div>
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
					<div className="bg-white/80 dark:bg-slate-900/80 p-6 border border-gray-200 dark:border-gray-700 rounded-xl backdrop-blur">
						<h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Top Performing Forms</h3>
						<div className="space-y-4">
							{[...forms]
								.sort((a, b) => b.responses - a.responses)
								.slice(0, 3)
								.map((form, index) => (
									<div key={form.id} className="flex items-center justify-between p-3 bg-white/70 dark:bg-slate-800/60 border border-gray-200 dark:border-gray-700 rounded-lg">
										<div className="flex items-center space-x-3">
											...
											<div>
												<p className="font-medium text-gray-900 dark:text-gray-100">{form.title}</p>
												<p className="text-sm text-gray-600 dark:text-gray-400">{form.responses} responses</p>
											</div>
										</div>
					...
					<div className="bg-white/80 dark:bg-slate-900/80 p-6 border border-gray-200 dark:border-gray-700 rounded-xl backdrop-blur">
						...
						<div className="space-y-4">
							<div className="flex items-center space-x-3 p-3 bg-white/70 dark:bg-slate-800/60 border border-gray-200 dark:border-gray-700 rounded-lg">
								...
								<div>
									<p className="text-sm text-gray-900 dark:text-gray-100">New response on Contact Form</p>
							...
							<div className="flex items-center space-x-3 p-3 bg-white/70 dark:bg-slate-800/60 border border-gray-200 dark:border-gray-700 rounded-lg">
								<Clock className="h-5 w-5 text-gray-400" />
								<div>
									<p className="text-sm text-gray-900 dark:text-gray-100">Form "Customer Survey" published</p>
							...
							<div className="flex items-center space-x-3 p-3 bg-white/70 dark:bg-slate-800/60 border border-gray-200 dark:border-gray-700 rounded-lg">
								<Clock className="h-5 w-5 text-gray-400" />
								<div>
									<p className="text-sm text-gray-900 dark:text-gray-100">5 new responses on Job Application</p>
	...
	return (
		<div className="h-[calc(100vh-64px)] bg-gray-50 dark:bg-slate-950 flex">
			{/* Sidebar */}
			<div className="w-64 bg-white/80 dark:bg-slate-900/80 border-r border-gray-200 dark:border-gray-700 flex flex-col backdrop-blur">
				<div className="p-6 border-b border-gray-200 dark:border-gray-700">
					<h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">AI Form Builder</h1>
					<p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Create forms with AI assistance</p>
				</div>
					...
					<div className="space-y-2">
						<button
							...
							className={`w-full flex items-center space-x-3 px-3 py-2 rounded-lg transition-colors ${
								activeTab === 'chat' ? 'bg-purple-100 text-purple-700 border border-purple-200' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-800'
							}`}
						>
							<Send size={18} />
							<span>AI Chat</span>
						...
						<button
							...
							className={`w-full flex items-center space-x-3 px-3 py-2 rounded-lg transition-colors ${
								activeTab === 'forms' ? 'bg-purple-100 text-purple-700 border border-purple-200' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-800'
							}`}
						>
							<FileText size={18} />
							<span>My Forms</span>
							<span className="ml-auto bg-gray-200 dark:bg-slate-800 text-gray-800 dark:text-gray-200 text-xs px-2 py-1 rounded-full">{forms.length}</span>
						</button>

						<button
							onClick={() => setActiveTab('analytics')}
							className={`w-full flex items-center space-x-3 px-3 py-2 rounded-lg transition-colors ${
								activeTab === 'analytics' ? 'bg-purple-100 text-purple-700 border border-purple-200' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-800'
							}`}
						>
							<BarChart3 size={18} />
							<span>Analytics</span>
						...
						<button className="w-full flex items-center space-x-3 px-3 py-2 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors" onClick={() => setActiveTab('settings')}>
